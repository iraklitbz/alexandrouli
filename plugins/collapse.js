export default function() {
    ...
    function initCollapse(element) {
      if ( element.triggers ) {
        ...
        updateTriggers(element, !Util.hasClass(element.element, 'hidden'));
    ...
    function toggleVisibility(element) {
      var bool = Util.hasClass(element.element, 'hidden');
    ...
    function animateElement(element, bool) {
      ...
      if(!element.animate || !window.requestAnimationFrame) {
        Util.toggleClass(element.element, 'hidden', !bool);
        element.animating = false;
        return;
      }

      // animate content height
      Util.removeClass(element.element, 'hidden');
      ...
      Util.setHeight(initHeight, finalHeight, element.element, 200, function(){
        if(!bool) Util.addClass(element.element, 'hidden');
